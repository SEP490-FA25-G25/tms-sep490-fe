...
export const subjectAdminApi = createApi({
  ...
  tagTypes: ["Subjects"],
  endpoints: (builder) => ({
    getSubjectSummaries: builder.query<
      SubjectSummary[],
      { status?: SubjectStatus; search?: string } | void
    >({
      query: (params) => {
      ...
      transformResponse: (response: ApiResponse<SubjectSummary[]>) =>
        response.data,
    ...
    getSubjectDetail: builder.query<SubjectDetail, number>({
      ...
      transformResponse: (response: ApiResponse<SubjectDetail>) =>
        response.data,
...
export const { useGetSubjectSummariesQuery, useGetSubjectDetailQuery } =
  subjectAdminApi;
