...
import { useGetTeacherClassSessionsMetricsQuery } from "@/store/services/attendanceApi";
import {
  SessionStatus,
...
import { Users, BookOpen, Loader2, AlertTriangle } from "lucide-react";
...
export function SessionsTab({ classId }: SessionsTabProps) {
  ...
  const {
    ...
    isLoading,
    error,
  } = useGetTeacherClassSessionsMetricsQuery(classId);
    ...
    switch (status) {
      ...
      case SessionStatus.PLANNED:
        return (
          <Badge
            variant="outline"
            className="bg-sky-100 text-sky-700 border-sky-200 hover:bg-sky-100/80"
          >
            {displayStatus}
          </Badge>
        );
      case SessionStatus.CANCELLED:
        return (
          <Badge
            variant="outline"
            className="bg-rose-100 text-rose-700 border-rose-200 hover:bg-rose-100/80"
          >
            {displayStatus}
          </Badge>
        );
