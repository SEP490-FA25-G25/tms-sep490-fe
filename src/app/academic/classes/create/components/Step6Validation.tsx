...
export function Step6Validation({ classId, onFinish }: Step6ValidationProps) {
  ...
  const [validateClass, { isLoading: isValidating }] =
    useValidateClassMutation();
  ...
  const canSubmit = Boolean(
    result?.canSubmit && result.valid && result.errors.length === 0
  );
  ...
  const infoRows = useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        value: classOverview.maxCapacity?.toString(),
      },
    ];
  }, [classOverview]);
  ...
  const collectTeacherNames = useCallback((source: unknown): string[] => {
    ...
    if (Array.isArray(source)) {
      ...
      if (Array.isArray(obj.teachers))
        collectTeacherNames(obj.teachers).forEach((n) => names.add(n));
    }
    return [...names];
  }, []);
  ...
  const filterSession = useCallback(
    (session: Record<string, unknown>) => {
      ...
      if (selectedStatus === "completed")
        return hasTimeSlot && hasResource && hasTeacher;
      return true;
    },
    [selectedStatus]
  );
  ...
  const timelineWeeks = useMemo(() => {
    const weeks =
      selectedWeek === "all"
        ? weekGroups
        : weekGroups.filter((w) => w.weekNumber === selectedWeek);
    return weeks
      .map((week) => ({
        ...week,
        sessions: week.sessions.filter((s) =>
          filterSession(s as unknown as Record<string, unknown>)
        ),
      }))
  ...
  const getTimeSlotLabel = (session: Record<string, unknown>) => {
    const start = collectStringsFromObject(session, [
      "startTime",
      "timeSlotStart",
    ]);
    ...
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Kiểm tra lớp học trước khi gửi duyệt • {summaryChecks.totalSessions}{" "}
          buổi
        </p>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4 p-4 rounded-lg border bg-muted/30">
        ...
        <div>
          ...
          <p className="text-xl font-semibold">
            {summaryChecks.sessionsWithTimeSlots}
          </p>
          {summaryChecks.sessionsWithoutTimeSlots > 0 && (
            <p className="text-xs text-red-600">
              {summaryChecks.sessionsWithoutTimeSlots} thiếu
            </p>
          )}
        </div>
        <div>
          ...
          <p className="text-xl font-semibold">
            {summaryChecks.sessionsWithResources}
          </p>
          {summaryChecks.sessionsWithoutResources > 0 && (
            <p className="text-xs text-red-600">
              {summaryChecks.sessionsWithoutResources} thiếu
            </p>
          )}
        </div>
        <div>
          ...
          <p className="text-xl font-semibold">
            {summaryChecks.sessionsWithTeachers}
          </p>
          {summaryChecks.sessionsWithoutTeachers > 0 && (
            <p className="text-xs text-red-600">
              {summaryChecks.sessionsWithoutTeachers} thiếu
            </p>
          )}
        </div>
      </div>
      ...
      <div className="flex items-center justify-between p-4 rounded-lg border">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Kiểm tra {summaryChecks.totalSessions} buổi học
          </p>
        </div>
          ...
          >
            {result.valid ? (
              <span className="flex items-center gap-2">
                ...
                {result.message || "Lớp học đã đầy đủ thông tin"}
              </span>
            ) : (
              result.message
            )}
      ...
      <div className="flex gap-4">
        <div className="w-40">
          <Select
            ...
            onValueChange={(v) =>
              setSelectedWeek(v === "all" ? "all" : Number(v))
            }
          >
            <SelectTrigger>
              <SelectValue />
      ...
      <div className="space-y-4">
        {!result ? (
          ...
          <Alert>
            <AlertDescription>
              Không có buổi học khớp với bộ lọc.
            </AlertDescription>
          </Alert>
        )}
      </div>
      ...
      <Card className="border border-border/70">
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">
            Kiểm tra lại các thông tin chính trước khi gửi duyệt.
          </p>
        </CardHeader>
        <CardContent className="grid gap-4 md:grid-cols-2">
          {isClassLoading ? (
            ...
            infoRows.map((row) => (
              <div
                key={row.label}
                className="rounded-xl border border-border/50 bg-card/60 p-3 text-sm"
              >
      ...
      <div className="flex items-center justify-between p-4 rounded-lg border bg-primary/5">
        <div>
          <p className="font-medium">
            {canSubmit
              ? "Lớp học đủ điều kiện gửi duyệt"
              : "Hoàn thành kiểm tra để gửi duyệt"}
          </p>
          <p className="text-sm text-muted-foreground">
            {canSubmit
              ? "Nhấn nút để gửi lớp đi phê duyệt"
              : "Vui lòng kiểm tra và đảm bảo tất cả thông tin đã đầy đủ"}
          </p>
        </div>
        <Button
