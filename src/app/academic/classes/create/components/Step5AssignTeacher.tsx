...
const resolveSessionDay = (session: {
  ...
  if (
    typeof session.dayOfWeekNumber === "number" &&
    Number.isFinite(session.dayOfWeekNumber)
  ) {
...
const TeacherCard = ({
  ...
  const generalScore = useMemo(
    () => getGeneralScore(teacher.skillDetails),
    [teacher.skillDetails]
  );
          ...
          <div className="shrink-0">
            <span
              className={cn(
                ...
                generalScore.level >= 8
                  ? "bg-emerald-100 text-emerald-700"
                  : generalScore.level >= 6
...
export function Step5AssignTeacher({
  ...
  const [assignTeacher, { isLoading: isSubmitting }] =
    useAssignTeacherMutation();
  ...
  const checkSpecializationMatch = (teacherData?: {
    specializations?: string[];
  }) => {
    if (!courseSubject)
      return { matches: true, teacherSpec: null, courseSubject: null };
    ...
    if (teacherSpecs.length === 0)
      return { matches: true, teacherSpec: null, courseSubject };
  ...
  const {
    ...
    { classId: classId ?? 0 },
    {
      skip: !classId || assignmentMode !== "single",
  ...
  const {
    ...
    { classId: classId ?? 0 },
    {
      skip: !classId || assignmentMode !== "multi",
      refetchOnMountOrArgChange: true,
    }
  );
  ...
  const isLoading =
    assignmentMode === "single" ? isSingleLoading : isDayLoading;
  ...
  const { recommended, unavailable } = useMemo(() => {
    const rec = teachers.filter(
      (t) => t.availabilityStatus === "FULLY_AVAILABLE"
    );
    const una = teachers.filter(
      (t) => t.availabilityStatus !== "FULLY_AVAILABLE"
    );
  ...
  const allowedDays = useMemo(
    () =>
      (scheduleDays.length ? [...scheduleDays] : [1, 2, 3, 4, 5, 6, 0]).sort(
        (a, b) => a - b
      ),
    [scheduleDays]
  );
  ...
  const teachersByDay = useMemo(() => {
    const map: Record<
      number,
      { info: TeacherDayAvailabilityInfo; teachers: DayTeacherEntry[] }
    > = {};
  ...
  const filteredDays = useMemo(
    () =>
      Object.keys(teachersByDay)
        .map(Number)
        .filter((d) => (sessionsByDay[d]?.length ?? 0) > 0),
    ...
    sessions.forEach((s) => {
      ...
      if (teachers.length > 0) {
        ...
        teachers.forEach((t) => {
          ...
          if (
            teacherId &&
            !result[day].some((existing) => existing.teacherId === teacherId)
          ) {
  ...
  const assignedSessions =
    sessionsData?.data?.sessions?.filter((s) => s.hasTeacher)?.length ?? 0;
  ...
  const requestAssign = (
    ...
    sessionIds: number[] | null = null,
    dayLabel?: string,
    skillDetails?: { specializations?: string[] }
  ) => {
    // Check for specialization mismatch
    const specCheck = skillDetails
      ? checkSpecializationMatch(skillDetails)
      : null;
    ...
    if (sessionIds) {
      ...
      const dayValue = Object.entries(sessionsByDay).find(
        ([, ids]) =>
          ids.length === sessionIds.length &&
          ids.every((id) => sessionIds.includes(id))
      )?.[0];
      ...
      if (hasExistingAssignment || hasSpecMismatch) {
        ...
        const uniqueNames = [
          ...new Set(allAssigned.map((t) => t.fullName)),
        ].filter(Boolean);
      ...
      {isLoading ? (
        ...
        <Fragment>
          ...
          {recommended.length > 0 ? (
            ...
            <Alert>
              <AlertDescription>
                Chưa có giáo viên đạt 100% khả dụng.
              </AlertDescription>
            </Alert>
          )}
            ...
            <div className="rounded-lg border bg-muted/30">
              ...
              {isUnavailableExpanded && (
                <div className="space-y-1 px-4 pb-4">
                  {unavailable.map((t) => (
                    <UnavailableTeacherItem key={t.teacherId} teacher={t} />
                  ))}
                </div>
              )}
            </div>
        ...
        <Alert>
          <AlertDescription>
            Chưa tìm thấy giáo viên có thể dạy toàn bộ các buổi của từng ngày.
          </AlertDescription>
        </Alert>
      ) : (
              ...
              >
                ...
                <div className="space-y-2">
                  {group.teachers.map((t) => {
                    ...
                    const generalSkill = t.skillDetails?.find(
                      (s) => s.skill === "GENERAL"
                    );
                    const generalScore = generalSkill
                      ? {
                          spec: generalSkill.specialization,
                          level: generalSkill.level,
                        }
                      : null;

                    // Fallback: get specialization from skills array if no skillDetails
                    const fallbackSpec =
                      !generalScore && t.skills?.length > 0
                        ? t.skills.find((s) => s !== "GENERAL") || t.skills[0]
                        : null;

                    // Check if this teacher is already assigned for this day
                    const isThisTeacherAssigned = assignedTeachers.some(
                      (at) => at.teacherId === t.teacherId
                    );
              ...
              {confirmDialog.specializationMismatch && (
                <div className="mt-3 p-3 rounded-lg bg-amber-50 border border-amber-200 space-y-1">
                  <p className="text-sm font-medium text-amber-800">
                    ⚠️ Chuyên môn không khớp
                  </p>
                  <p className="text-sm text-amber-700">
                    Giáo viên{" "}
                    <span className="font-semibold">
                      {confirmDialog.teacherName}
                    </span>{" "}
                    có chuyên môn{" "}
                    <span className="font-semibold">
                      {confirmDialog.specializationMismatch.teacherSpec ||
                        "Chung"}
                    </span>
                    , nhưng khóa học này yêu cầu{" "}
                    <span className="font-semibold">
                      {confirmDialog.specializationMismatch.courseSubject}
                    </span>
                    .
                  </p>
                </div>
            ...
            <AlertDialogAction
              onClick={() => {
                if (confirmDialog.teacherId !== null) {
                  handleAssign(
                    confirmDialog.teacherId,
                    confirmDialog.sessionIds
                  );
                }
              }}
            >
